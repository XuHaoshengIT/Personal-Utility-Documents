...
的确，需要多费点事（多加空格）来产生 `` ，但是简单地「每个换行都转换为 ``」的方法在 Markdown 中并不适合， Markdown 中 email 式的 [区块引用](http://wowubuntu.com/markdown/#blockquote) 和多段落的 [列表](http://wowubuntu.com/markdown/#list) 在使用换行来排版的时候，不但更好用，还更方便阅读。
